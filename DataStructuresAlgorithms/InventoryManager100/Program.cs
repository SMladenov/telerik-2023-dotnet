...
namespace InventoryManager100
{
    internal class Program
    {
        ...
        {
            string terminaryComand = "end";
            string input = Console.ReadLine();

            while (input != terminaryComand)
            {
        ...
        static void AddCommand(string[] commandParams)
        {
            ...
            if (TryAdd(item))
            {
                sb.Append($"Ok: Item {itemName} added successfully");
            }
            else
            {
                sb.Append($"Error: Item {itemName} already exists");
            }
        }
        ...
        static void FilterCommand(string[] commandParams)
        {
            ...
            if (commandParams[2] == "type")
            {
                ...
                {
                    items = FilterByType(commandParams[2]);
                }
            }
