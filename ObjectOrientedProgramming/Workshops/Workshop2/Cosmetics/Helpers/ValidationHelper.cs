...
namespace Cosmetics.Helpers
{
    public class ValidationHelper
    {
        ...
        public static void ValidateMillilitres(string value)
        {
            if (int.TryParse(value, out int number))
            {
                if (number < 0)
                {
                    throw new ArgumentException("Millilitres cannot be negative");
                }
            }
            else
            {
                throw new ArgumentException("Invalid millilitres");
            }
        }
    }
}
